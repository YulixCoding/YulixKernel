...
bool nullhook::call_kernel_function(void* kernel_function_adress)
{
	if (!kernel_function_adress)
		return false;
	...
	BYTE shell_code[] = { 0x48, 0xB8 }; //mov rax, xxx	//Change the shell code for undetected , Currently only undetected for VAC
	BYTE shell_code_end[] = { 0xFF, 0xE0 }; //jmp rax	//Change the shell code for undetected , Currently only undetected for VAC

	RtlSecureZeroMemory(&orig, sizeof(orig));
	memcpy((PVOID)((ULONG_PTR)orig), &shell_code, sizeof(shell_code));
	uintptr_t hook_address = reinterpret_cast<uintptr_t>(kernel_function_adress);
	...
	NtGdiDeleteObjectApp = (DeleteObjectApp_t)get_system_module_export(L"win32kfull.sys", "NtGdiDeleteObjectApp");

	DebugMessage("writeToReadOnlyMemory: True");
